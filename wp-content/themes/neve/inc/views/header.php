...
class Header extends Base_View {
	...
	public function init() {
		...
		add_action( 'wp_enqueue_scripts', array( $this, 'conditionally_hide_login_form' ) );
	...
	/**
	 * Conditionally hide login forms on pages where they shouldn't appear.
	 */
	public function conditionally_hide_login_form() {
		// Only hide login forms if we're not on allowed pages
		if ( $this->is_login_register_page() ) {
			return; // Don't hide on allowed pages
		}

		// CSS to hide common login form selectors
		$hide_login_css = '
		/* Hide Ultimate Member login forms */
		.um-login,
		.um-login form,
	

		/* Hide general login forms */
		.login-form,
		.loginform,
		#loginform,
		.user-login-form,
		...
		wp_add_inline_style( 'neve-style', $hide_login_css );
	}
	...
	private function get_last_menu_items_markup( $items = '' ) {
		...
		foreach ( $additional_items as $item ) {
			if ( $item === 'search' ) {
				...
				// Only show login form on login/register pages
				if ( $this->is_login_register_page() ) {
	 ...
	 * @return bool
	 */
	private function is_login_register_page() {
		// Check for WooCommerce my account page
		if ( function_exists( 'is_account_page' ) && is_account_page() ) {
			return true;
		}

		// Check for custom login/register pages
		if ( is_page_template( 'page-login.php' ) || is_page_template( 'page-register.php' ) ) {
			return true;
		}

		// Check by page slug - specific pages where login should show
		global $post;
		if ( $post && in_array( $post->post_name, array( 'login', 'register', 'sterlings-app', 'view-report', 'order-report' ), true ) ) {
			return true;
		}
