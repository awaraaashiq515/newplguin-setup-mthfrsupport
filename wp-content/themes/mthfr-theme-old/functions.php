...
function custom_filter_table_api(WP_REST_Request $request) {
    ...
    $result_id = intval($request->get_param('result_id'));  // Fetch upload_id from request

    try {
        // Step 1: Fetch the correct report_path using upload_id
        if (!$result_id) {
        ...
        // Get config for current report type (default to 'basic' if not found)
        $config = $report_configs[$report_info->report_type] ?? $report_configs['Excipient'];

        // Step 2: Load JSON data from the user's report
...
// Extract the variants array from the JSON structure
$data = $json_data['variants'] ?? [];

if (empty($data)) {
    return new WP_Error('no_variants', 'No variants found in report.', ['status' => 404]);
}
        // Step 3: Group data by rs10306114
        $grouped_data = [];
        foreach ($data as $row) {
            // Ensure $row is an array before accessing it
            if (!is_array($row)) {
        ...
        // Fixed the array mapping to handle potential data structure issues
        $filteredData = [];
        foreach ($data as $entry) {
            // Ensure $entry is an array before accessing it
            if (!is_array($entry)) {
                continue; // Skip non-array entries
            }
            ...
            $filteredData[] = [
                ...
                "Info" => $entry["Info"] ?? "",
                'Video' => $entry['Video'] ?? "",
                'Group' => $entry['Group'] ?? ($entry['rs10306114'] ?? ""),
                'Tags' => $entry['Tags'] ?? ""  // Added null coalescing operator for safety
            ];
        }
        ...
        $response_data = [
            'body_html'      => '', // You had $html but it's not defined, using empty string
            'isSearchResult' => (!empty($tags) || !empty($snps)),
            'data' => $filteredData
        ];

        // You referenced $header_html but it's not defined, commenting out for now
        // if (!empty($header_html)) {
        //     $response_data['header_html'] = $header_html;
        // }
...
function report_visualization_shortcode($atts) {
    ...
    jQuery(document).ready(function($) {
        ...
        const enableTags = <?php echo json_encode($config['enable_tags']); ?>;  // PHP variable passed to JavaScript
        ...
        function changeImage(imageUrl) {
            $('.diagram-image').attr('src', 'https://mthfrsupport.org/wp-content/uploads/pathway-images/' + imageUrl);
        }
...
function get_user_results() {
    ...
    // Fetch user uploads from the database
    global $wpdb;
    $table_name = $wpdb->prefix . 'user_reports'; // Replace with your table name
    $uploads = $wpdb->get_results(
        $wpdb->prepare(
            "SELECT u.id, u.upload_id, u.report_path, u.report_type, u.report_name, u.updated_at, u2.file_name
            FROM $table_name u
            INNER JOIN {$wpdb->prefix}user_uploads u2 ON u.upload_id = u2.id
            WHERE u2.user_id = %d AND u.status = 'completed'
    ...
    if ($uploads) {
        ...
        $uploads_data = array_map(function ($upload) {
            $file_name = basename($upload['file_name']); // Extract file name
        $report_name = 'MTHFRSupport ' . $upload['report_name'];   // Assuming 'report_type' corresponds to 'report_name'
            $created_at = $upload['updated_at']; // Get created_at timestamp

            return [
                'result_id' => $upload['id'],
                'report_name' => $report_name,
                'file_name' => $file_name,
              //  'created_at' => date('Y-m-d', strtotime($created_at)), // Format date to Y-m-d
                'created_at' => $created_at,
                'upload_id' => $upload['upload_id'],
            ];
        }, $uploads);
        wp_send_json_success(['results' => $uploads_data]);
    }
}
...
// Process PDF creation after order is completed
...
//             $result = create_pdf_from_upload($upload_id,$order_id, $product_id, $product_name);  // Pass 'variant' as the variant value
...
// function create_pdf_from_upload($upload_id,$order_id, $product_id, $product_name) {
