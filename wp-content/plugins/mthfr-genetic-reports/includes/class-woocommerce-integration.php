...
add_action('woocommerce_checkout_create_order_line_item', array($this, 'save_upload_id_to_order_item'), 10, 4);
    ...
    /**
     * FIXED: AJAX handler for ASYNC report generation (queues job instead of generating immediately)
     */
    public function ajax_test_report() {
        ...
        error_log("MTHFR: Queueing ASYNC report generation for order {$order_id}");
        ...
        try {
            ...
            // Check if async report generator is available
            if (!class_exists('MTHFR_Async_Report_Generator')) {
                wp_send_json_error(array('message' => 'Async Report Generator class not found'));
                return;
            }
            ...
            // Queue ASYNC report generation instead of generating immediately
            $action_id = MTHFR_Async_Report_Generator::schedule_report_generation(
                ...
                $has_subscription,
                array(
                    'notify_email' => true, // Send email when complete
                    'manual_trigger' => true // Mark as manually triggered
                )
            );

            if ($action_id) {
                wp_send_json_success(array(
                    'message' => 'Report generation queued successfully! You will receive an email when it\'s ready.',
                    'action_id' => $action_id,
                    'upload_id' => $upload_id,
                    'order_id' => $order_id,
                ...
                wp_send_json_error(array('message' => 'Failed to queue report generation job'));
            }

        } catch (Exception $e) {
            error_log('MTHFR Async Report Queue Error: ' . $e->getMessage());
            wp_send_json_error(array('message' => 'Exception: ' . $e->getMessage()));
        }
    }
    
    /**
     * FIXED: AJAX handler for ASYNC PDF generation (queues job instead of generating immediately)
     */
    public function ajax_generate_pdf() {
      ...
      if (!isset($_POST['nonce']) || (!wp_verify_nonce($_POST['nonce'], 'mthfr_test_action') && !wp_verify_nonce($_POST['nonce'], 'mthfr_working_action'))) {
        ...
        error_log("MTHFR: Queueing ASYNC PDF generation for order {$order_id}");
        ...
        try {
            ...
            // Queue ASYNC PDF generation instead of generating immediately
            $action_id = MTHFR_Async_Report_Generator::schedule_report_generation(
                $upload_id,
                $order_id,
                $product_name,
                ...
                array(
                    'notify_email' => true, // Send email when complete
                    'manual_trigger' => true, // Mark as manually triggered
                    'pdf_only' => true // Focus on PDF generation
                )
            );

            if ($action_id) {
                wp_send_json_success(array(
                    'message' => 'PDF generation queued successfully! You will receive an email when it\'s ready.',
                    'action_id' => $action_id,
                    'upload_id' => $upload_id,
                    'order_id' => $order_id,
                    'product_name' => $product_name,
                ...
                wp_send_json_error(array('message' => 'Failed to queue PDF generation job'));
            }

        } catch (Exception $e) {
            error_log('MTHFR Async PDF Queue Error: ' . $e->getMessage());
            wp_send_json_error(array('message' => 'Exception: ' . $e->getMessage()));
        }
    }
...
function mthfr_get_download_urls($order_id,$result=null) {
    ...
    // If result is provided with paths, use them directly
    if ($result && !empty($result['json_report_path'])) {
        $json_url = str_replace($upload_dir['basedir'], $upload_dir['baseurl'], $result['json_report_path']);
    }

    if ($result && !empty($result['pdf_report_path'])) {
        $pdf_url = str_replace($upload_dir['basedir'], $upload_dir['baseurl'], $result['pdf_report_path']);
    }

    // Fallback to database if result not provided or paths missing
    if ((!$json_url || !$pdf_url) && class_exists('MTHFR_Database')) {
        $report = MTHFR_Database::get_report_by_order($order_id);

        if ($report) {
            $upload_id = $report->upload_id ?? null;

            if ($upload_id) {
                // Build structured path for JSON report: /user_reports/upload_{id}/filename
                if (!$json_url && !empty($report->report_path)) {
                    $filename = basename($report->report_path);
                    $relative_path = '/user_reports/upload_' . $upload_id . '/' . $filename;
                    $full_path = $upload_dir['basedir'] . $relative_path;

                    // Validate file existence
                    if (file_exists($full_path)) {
                        $json_url = $upload_dir['baseurl'] . $relative_path;
                    }
                }

                // Build structured path for PDF report: /user_reports/upload_{id}/filename
                if (!$pdf_url && !empty($report->pdf_report)) {
    ...
    return array(
        'json_url' => $json_url,
        'pdf_url' => $pdf_url,
        'report_type' => ($result['report_type'] ?? null) ?: 'Unknown',
        'status' => 'completed',
        'created_at' => ($result['timestamp'] ?? null) ?: current_time('c')
    );
}
...
add_action('add_meta_boxes', function($post_type) {
    ...
    /**
     * Process order completion and generate reports asynchronously
     */
     function process_order_completion($order_id) {
        
        error_log("MTHFR: Processing order completion for order: $order_id - HOOK FIRED");
        ...
        foreach ($order->get_items() as $item_id => $item) {
            ...
            if ($upload_id && in_array($product_id, $product_ids_to_check)) {
                error_log("MTHFR: Scheduling synchronous report generation for upload ID: $upload_id");

                // Check if report generator is available
                if (!class_exists('MTHFR_Report_Generator')) {
                    error_log("MTHFR: MTHFR_Report_Generator class not found, cannot generate report");
                    $order->add_order_note(
                        sprintf('MTHFR report generation failed for %s (Upload ID: %d) - Report Generator not available', $product_name, $upload_id)
                    );
                    continue;
                }

                // Store scheduling details in order item meta
                $item->add_meta_data('_mthfr_report_scheduled', 'yes');
                $item->add_meta_data('_mthfr_report_date_scheduled', current_time('mysql'));
                $item->save();

                // Add order note indicating scheduled processing
                $order->add_order_note(
                    sprintf('MTHFR report generation scheduled for %s (Upload ID: %d). Report will be generated after checkout completion.', $product_name, $upload_id)
                );

                // Schedule synchronous report generation after response is sent
                register_shutdown_function(function() use ($upload_id, $order_id, $product_name, $has_subscription, $item, $order) {
                    ...
                    if ($result && isset($result['success']) && $result['success']) {
                        ...
                        $item->add_meta_data('_mthfr_report_date_generated', current_time('mysql'));
                        $item->save();
                        ...
                        error_log("MTHFR: Report generated successfully for upload ID: $upload_id");
                    } else {
                        $error = isset($result['error']) ? $result['error'] : 'Unknown error';
                        $item->add_meta_data('_mthfr_report_generated', 'failed');
                        $item->add_meta_data('_mthfr_report_error', $error);
                        $item->save();

                        $order->add_order_note(
                            sprintf('MTHFR report generation failed for %s (Upload ID: %d) - %s', $product_name, $upload_id, $error)
                        );

                        error_log("MTHFR: Report generation failed for upload ID: $upload_id - $error");
                    }
                });

                error_log("MTHFR: Synchronous report generation scheduled successfully for upload ID: $upload_id");
            } else {
                error_log("MTHFR: Skipping item - no upload_id or invalid product ID");
            }
        }
    }
    /**
